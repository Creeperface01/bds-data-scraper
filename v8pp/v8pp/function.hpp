...
namespace v8pp {
    ...
    namespace detail {
        ...
        template<typename T>
        typename std::enable_if<is_pointer_cast_allowed<T>::value, v8::Local < v8::Value>>
        ...
        typename function_traits<F>::return_type
                invoke(v8::FunctionCallbackInfo < v8::Value >
        const& args, std::false_type /*is_member_function_pointer*/) {
        return
        call_from_v8<Traits, F>(std::forward<F>(get_external_data<F>(args.Data())), args
        );
    }
    ...
    typename function_traits<F>::return_type
            invoke(v8::FunctionCallbackInfo < v8::Value >
    const& args, std::true_type /*is_member_function_pointer*/) {
...
return
call_from_v8<Traits, class_type, F>(*ptr, std::forward<F>(get_external_data<F>(args.Data())), args
);
}

template<typename Traits, typename F>
void forward_ret(v8::FunctionCallbackInfo < v8::Value >
const& args, std::true_type /*is_void_return*/)
{
...
template<typename Traits, typename F>
void forward_ret(v8::FunctionCallbackInfo < v8::Value >
const& args, std::false_type /*is_void_return*/)
{
...
.
Set(converter::to_v8(args.GetIsolate(),
                     invoke<Traits, F>(args, std::is_member_function_pointer<F>()))
);
}

template<typename Traits, typename F>
void forward_function(v8::FunctionCallbackInfo < v8::Value >
const& args)
{
...
v8::Local <v8::Function> wrap_function(v8::Isolate *isolate,
    ...
    v8::Local <v8::Function> fn = v8::Function::New(isolate->GetCurrentContext(),
                                                    &detail::forward_function < Traits, F_type > ,
                                                    detail::set_external_data(isolate, std::forward<F_type>(
                                                            func))).ToLocalChecked();
    if (name && *name) {
