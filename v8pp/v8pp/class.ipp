...
namespace v8pp {
    ...
    namespace detail {
        ...
        static V8PP_IMPL std::string

        pointer_str(void const *ptr) {
...
//
        ...
        V8PP_IMPL std::string

        class_info::class_name() const {
...
//
        ...
        V8PP_IMPL
        object_registry<Traits>::object_registry(v8::Isolate *isolate, type_info const &type, dtor_function &&dtor)
                : class_info(type, type_id<Traits>()), isolate_(isolate),
            ...
            v8::Local <v8::FunctionTemplate> js_func = v8::FunctionTemplate::New(isolate_,
                                                                                 [](v8::FunctionCallbackInfo <v8::Value> const &args) {
                                                                                     ...
                                                                                     object_registry *
                                                                                     this_ = get_external_data<object_registry *>(
                                                                                             args.Data());
                                                                                     try {
        ...
        object_registry<Traits>::find_object(object_id
        id,
...
template<typename Traits>
V8PP_IMPL v8::Local<v8::Object>
object_registry<Traits>::find_v8_object(pointer_type
const& ptr) const
{
...
template<typename Traits>
V8PP_IMPL v8::Local<v8::Object>
object_registry<Traits>::wrap_object(pointer_type
const& object,
bool call_dtor
)
{
...
pointer_str (Traits::pointer_id(object))
...
objects_.
emplace(object, wrapped_object{std::move(pobj), call_dtor}
);
...
template<typename Traits>
V8PP_IMPL v8::Local<v8::Object>
object_registry<Traits>::wrap_object(v8::FunctionCallbackInfo<v8::Value>
const& args)
{
if (!ctor_)
{
...
template<typename Traits>
V8PP_IMPL typename object_registry<Traits>::pointer_type
object_registry<Traits>::unwrap_object(v8::Local<v8::Value>
value)
{
v8::HandleScope scope(isolate_);
...
//
template<typename Traits>
V8PP_IMPL object_registry<Traits>
&
classes::add(v8::Isolate
* isolate,
type_info const &type,
...
throw
std::runtime_error((*it)
->

class_name()

+ " is already exist in isolate " +
pointer_str(isolate)
);
}
info->classes_.emplace_back(new
object_registry<Traits>(isolate, type, std::move(dtor)
));
return *static_cast<object_registry<Traits>*>(info->classes_.
...
return *static_cast<object_registry<Traits>*>(it->
...
throw
std::runtime_error(class_info(type, traits)
.

class_name()

+ " is not registered in isolate " +
pointer_str(isolate)
);
}
...
V8PP_IMPL classes::classes_info::iterator
classes::find(type_info
const& type)
{
return
std::find_if(classes_
.
...
[&type](
classes_info::value_type const &info
)
{
return info->type ==
type;
